    ...
    for _ in range(MAX_PAGES):
        ...
        for item in items:
            ...
            all_notices[f"wb-{nid}"] = {
                ...
                # strengthening" - 나이지리아 SPIN 수력 PPP 사례)이 나오는 경우가 있어서
                # 필터는 원문 전체를 봐야 한다.
                # 다만 원문 전체를 그대로 INCLUDE/EXCLUDE 판단에 쓰면, 공고 맨 끝에 항상
                # 붙는 담당기관 제출처 주소("Corner of Nationalist Road..." 같은 문구)의
                # 거리 이름이 "road" 등 INCLUDE 키워드에 우연히 걸리는 문제가 있었다
                # (잠비아 DZAP 사례). 그래서 앞부분만 보는 길이 제한은 정확도가 훨씬
                # 중요한 HARD_EXCLUDE에는 적용하지 말고, INCLUDE/EXCLUDE(소프트)에만
                # 적용하도록 _sector_filter.is_relevant()에서 처리한다(_RELEVANCE_HEAD_CHARS).
                # 그래서 여기서는 자르지 않고 원문 전체를 그대로 넘긴다.
